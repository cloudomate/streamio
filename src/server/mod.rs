//! HTTP and WebSocket server for signaling and client serving

use crate::renderer::{HorizonRenderer, InputEvent};
use crate::streamer::{SignalingMessage, WebRtcStreamer};
use anyhow::Result;
use axum::{
    extract::{
...
async fn handle_websocket(socket: WebSocket, state: Arc<AppState>) {
    ...
    let streamer = match WebRtcStreamer::new(state.width, state.height, state.fps, sig_tx) {
        Ok(s) => Arc::new(s),
        Err(e) => {
    ...
    if let Err(e) = streamer.start() {
        tracing::error!("Failed to start streamer: {}", e);
        return;
    }
    ...
    // Create offer after a short delay (let pipeline settle)
        ...
        loop {
            ...
            match renderer.render_frame().await {
                Ok(rgba_data) => {
                    ...
                    if let Err(e) = streamer_render.push_frame(&rgba_data, pts) {
